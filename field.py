...
class Field:
    def __init__(self, screen, cell_size: tuple | list, field_size: tuple | list = (5, 8), cell_margin: int = 3):
        ...
        #               [copy(c), copy(c), copy(c), copy(c), copy(c)]]
        self.stack = []
        self.init_values = (2, 4, 8, 16, 32, 64, 128)
        self.fill_randomly()
    ...
    def fill_randomly(self) -> None:
        ...
        self.field = [[Cell(screen=self.screen, position=(0, 0), indexes=(row, cell), size=self.cell_size, value=self.init_values[random.randint(0, 6)]) for cell in range(self.field_size[0])] for row in range(self.field_size[1])]
